...
beta_post_vi = posteriors_dictionary_sample['beta'][-1000:]
sigma2_post_vi = posteriors_dictionary_sample['sigma2'][-1000:]
eta_post_vi = np.sqrt(posteriors_dictionary_sample['eta'])[-1000:]
ls0_post_vi = posteriors_dictionary_sample['ls'][-1000:,0]
ls1_post_vi = posteriors_dictionary_sample['ls'][-1000:,1]
...
beta_mc = idata.posterior["beta"].values
sigma2_mc = idata.posterior["sigma2"].values
eta_mc = idata.posterior["eta"].values
ls_mc = idata.posterior["ls"].values
ls0_mc = ls_mc[:,:,0]
ls1_mc = ls_mc[:,:,1]
...
sigma2_post_naf = torch.exp(post_spl[:, 0]).squeeze().detach().numpy()
beta_post_naf = post_spl[:, 1].squeeze().detach().numpy()
eta_post_naf = np.sqrt(torch.exp(post_spl[:, 2]).squeeze().detach().numpy())
ls0_post_naf = torch.exp(post_spl[:, 3]).squeeze().detach().numpy()
ls1_post_naf = torch.exp(post_spl[:, 4]).squeeze().detach().numpy()
post_sample_bma_naf = np.column_stack((beta_post, sigma2_post, eta_post, ls0_post_naf, ls1_post_naf))

# Create Figure 4
# Create a 1 by 4 grid of subplots with the correct size for LaTeX insertion (3 inches width)
fig, axes = plt.subplots(1, 5, figsize=(22, 6))

# Plot 1
sns.kdeplot(beta_post_naf, label='NAFBMA', bw_adjust=6, linestyle='-', ax=axes[0], color='red', linewidth=3)
sns.kdeplot(beta_post_vi, label='VBMA', bw_adjust=6, linestyle=':', ax=axes[0], color='blue', linewidth=3)
sns.kdeplot(beta_mc[3,:], label='MCMC', bw_adjust=6, linestyle='--', ax=axes[0], color='black', linewidth=3)
axes[0].set_xlabel(r"$\beta$", fontsize=25)
axes[0].set_ylabel('Density', fontsize=25)

# Plot 2
sns.kdeplot(sigma2_post_naf, label='NAFBMA', bw_adjust=6, linestyle='-', ax=axes[1], color='red', linewidth=3)
sns.kdeplot(sigma2_post_vi, label='VBMA', bw_adjust=6, linestyle=':', ax=axes[1], color='blue', linewidth=3)
sns.kdeplot(sigma2_mc[3,:], label='MCMC', bw_adjust=6, linestyle='--', ax=axes[1], color='black', linewidth=3)
axes[1].set_xlabel(r"$\sigma^2$", fontsize=25)
axes[1].set_ylabel('')


# Plot 3
sns.kdeplot(eta_post_naf, label='NAFBMA', bw_adjust=6, linestyle='-', ax=axes[2], color='red', linewidth=3)
sns.kdeplot(eta_post_vi, label='VBMA', bw_adjust=6, linestyle=':', ax=axes[2], color='blue', linewidth=3)
sns.kdeplot((eta_mc[1,:]), label='MCMC', bw_adjust=6, linestyle='--', ax=axes[2], color='black', linewidth=3)
axes[2].set_xlabel(r"$\eta$", fontsize=25)
axes[2].set_ylabel('', fontsize=25)

# Plot 4
sns.kdeplot(ls0_post_naf, label='NAFBMA', bw_adjust=6, linestyle='-', ax=axes[3], color='red', linewidth=3)
sns.kdeplot(ls0_post_vi, label='VBMA', bw_adjust=6, linestyle=':', ax=axes[3], color='blue', linewidth=3)
sns.kdeplot(ls0_mc[3,:], label='MCMC', bw_adjust=6, linestyle='--', ax=axes[3], color='black', linewidth=3)
axes[3].set_xlabel(r"$\nu_Z$", fontsize=25)
axes[3].set_ylabel('')

# Plot 5
sns.kdeplot(ls1_post_naf, label='NAFBMA', bw_adjust=6, linestyle='-', ax=axes[4], color='red', linewidth=3)
sns.kdeplot(ls1_post_vi, label='VBMA', bw_adjust=6, linestyle=':', ax=axes[4], color='blue', linewidth=3)
sns.kdeplot(ls1_mc[3,:], label='MCMC', bw_adjust=6, linestyle='--', ax=axes[4], color='black', linewidth=3)
axes[4].set_xlabel(r"$\nu_N$", fontsize=25)
axes[4].set_ylabel('')
axes[4].legend(loc="upper right", bbox_to_anchor=(1.03, 1.0), fontsize=15, frameon=False)
