...
class BaseFlow(Module):
    ...
    def sample(self, n=1, context=None, **kwargs):      #这里只定义了一个方法就是sample，dim这个参数是从Module里继承来的，可以查看原始代码
        dim = self.dim
...
class SigmoidFlow(BaseFlow):
    ...
    def forward(self, x, logdet, dsparams, mollify=0.0, delta=nn_.delta):
        ...
        ndim = self.num_ds_dim                                #这个应该就是DSF的hidden layer那一层有多少个neuron
...
class SigmoidFlow_fix_lm(BaseFlow):
    def __init__(self, num_ds_dim=4):
        super(SigmoidFlow_fix_lm, self).__init__()
        self.num_ds_dim = num_ds_dim

        self.act_a = lambda x: nn.functional.softplus(x)
        self.act_b = lambda x: x
        self.act_w = lambda x: nn.functional.softmax(x,dim=2)
    ...
    def forward(self, x, logdet, dsparams, mollify=0.0, delta=nn_.delta):
        ...
        # Reshape x to have an additional dimension of size 1
        x = x.unsqueeze(-1)
        pre_sigm = a * x + b
        sigm = torch.sigmoid(pre_sigm)
        x_pre = torch.sum(w*sigm, dim=2)
        x_pre_clipped = x_pre * (1-delta) + delta * 0.5
        x_ = torch.log(x_pre_clipped) - torch.log(1-x_pre_clipped)
        xnew = x_

        logj = torch.log_softmax(dsparams[:,:,2*ndim:3*ndim], dim=2) + \
            nn_.logsigmoid(pre_sigm) + \
            nn_.logsigmoid(-pre_sigm) + torch.log(a)
