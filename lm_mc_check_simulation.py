...
def lm_simulation_nmc_check(data_size=100, rho=0.0, num_repeats=10, d_c=32, d_t=16, l_c=2, l_t=2, n_mc=16):
    """
    ...
    Args:
        ...
        n_mc (int): Number of Monte Carlo samples for gradient estimation.
    """
    ...
    results_save_path = f'simulation/lm/experiment_results/n_mc_check'
    ...
    np.savez(f'{results_save_path}/aggregated_results_n{data_size}_rho{rho_str}_dc{d_c}_dt{d_t}_lc{l_c}_lt{l_t}_nmc{n_mc}.npz',
    ...
    np.savez(f'{results_save_path}/raw_results_n{data_size}_rho{rho_str}_dc{d_c}_dt{d_t}_lc{l_c}_lt{l_t}_nmc{n_mc}.npz',
...
if __name__ == "__main__":
    # Example usage
    lm_simulation_nmc_check(data_size=100, rho=0.0, num_repeats=3, d_c=32, d_t=16, l_c=2, l_t=2, n_mc=16)
